...
__global__ void spmvJDSKernel(float *out, int *matColStart, int *matCols,
  ...
  {
      float dot = 0;
      //int row_start = matRows[row];
      int row_end =   matRows[row];
      for (int elem = 0; elem < row_end; elem++)
        dot += matData[matColStart[elem] + row] * vec[matCols[matColStart[elem] + row]];
...
static void spmvJDS(float *out, int *matColStart, int *matCols,
  ...
  spmvJDSKernel<<<ceil(dim/(float)TILE_WIDTH), TILE_WIDTH>>>(
