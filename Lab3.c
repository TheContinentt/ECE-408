...
__global__ void matrixMultiplyShared(float *A, float *B, float *C,
  ...
  {
    sharedM[ty][tx] = A[r * numAColumns + i * TILE_WIDTH + tx];
    sharedN[ty][tx] = B[(i * TILE_WIDTH + ty) * numBColumns + c];
    __syncthreads();
...
int main(int argc, char **argv) {
  ...
  dim3 dimGrid(ceil(numCColumns * 1.0 / double(TILE_WIDTH)), ceil(numCRows * 1.0 / double(TILE_WIDTH)), 1.0);
