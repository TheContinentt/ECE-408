...
__global__  void total(float *input, float *output, int len) {
  ...
  //@@ correct index
  __shared__ float SharedSum[2 * BLOCK_SIZE];

  int tx = threadIdx.x;
  int start = 2 * blockIdx.x * BLOCK_SIZE;
  if (start + tx < len)
    SharedSum[tx] = input[start + tx];
  else
    SharedSum[tx] = 0;
  if (start + BLOCK_SIZE + tx < len)
    SharedSum[BLOCK_SIZE + tx] = input[start + BLOCK_SIZE + tx];
  else
    SharedSum[BLOCK_SIZE + tx] = 0;

  for (int stride = BLOCK_SIZE; stride >= 1; stride >>= 1)
  {
    __syncthreads();
    if (tx < stride)
      SharedSum[tx] += SharedSum[tx + stride];
  }
  if (tx == 0)
...
int main(int argc, char **argv) {
  ...
  dim3 dimGrid(numOutputElements, 1.0, 1.0);
